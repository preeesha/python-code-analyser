   ...
   if st.button("Submit"):  
       if dir_path:
           ...
           if os.path.isdir(dir_path):
               ...
               st.success(f"Directory path received: `{dir_path}`")
               copy_local_dir(dir_path, "../testing")
               ingestion_pipeline(["../testing"],"py")
           else:
               st.error(f"❌ The path `{dir_path}` is not a valid directory.")
       else:
            st.warning("⚠️ Please enter a valid directory path.", "warning")
